...
try {
            ...
            $stmt = $conn->prepare("DELETE FROM Program2 WHERE Program_Name = :program_name;");

            $stmt->bindParam(':program_name', $program_name);

            $stmt->execute();

            $message = "You have successfully deleted $program_name as a registered program!";
...
<div class="container">
    ...
    <p>Warning: If you delete a program name from the table. All of the assigned functional areas will be deleted too.</p>
    </br></br>

    <form class="form-horizontal" role="form" method="post" action="Delete_Program.php">
        <div class="row">
            <div class="form-group" style="padding-bottom: 10px">
                <label for="program_name" class="col-sm-2 control-label">Program Name</label>
                <div class="col-sm-4">
                     <select class="form-control" id="program_name" name="program_name" required="true">
                          <?php
                    ...
                    <!-- <?php echo "<p class='text-danger'>$program_name</p>";?>   -->
                </div>
            </div>
        </div>
